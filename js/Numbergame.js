...
jQuery(function($){
	...
	{
	case 0:
		var startingVal = [0,0,0,ruleValue];
		document.getelementById("rules").innerHTML="The rule today is: add "+ruleValue;
		break;
	case 1:
		var startingVal = [1,1,1,ruleValue];
		document.getelementById("rules").innerHTML="The rule today is: multiply by  "+ruleValue;
		break;
	case 2:
		var startingVal = [ruleValue*10,ruleValue*10,ruleValue*10,0];
		document.getelementById("rules").innerHTML="The rule today is: subtract "+ruleValue;
		break;
	case 3:
		var temp = Math.pow(rulevalue, 10);
		var startingVal = [ruleValue*10,ruleValue*10,ruleValue*10,1];
		document.getelementById("rules").innerHTML="The rule today is: divide by "+ruleValue;
		break;
	default :
	...
	function gamestart()
	{
			...
			{
			case 0:
				startingVal = [1,1,1,1+ruleValue];
				document.getElementById("rules").innerHTML="The rule today is: add "+ruleValue +".<br> You win if you manage to create a tile showing : "+(ruleValue*10 +1);
				break;
			case 1:
				startingVal = [1,1,1,ruleValue,ruleValue,ruleValue,ruleValue];
				document.getElementById("rules").innerHTML="The rule today is: multiply by  "+ruleValue +".<br> You win if you manage to create a tile showing : "+Math.pow(ruleValue, 10) ;
				break;
			case 2:
				startingVal = [ruleValue*10+1,ruleValue*10+1,ruleValue*10+1,ruleValue*9+1];
				document.getElementById("rules").innerHTML="The rule today is: subtract "+ruleValue +".<br> You win if you manage to create a tile showing : 1.";
				break;
			case 3:
				var temp = Math.pow(ruleValue, 10);
				startingVal = [temp,temp,temp,temp/ ruleValue];
				document.getElementById("rules").innerHTML="The rule today is: divide by "+ruleValue +".<br> You win if you manage to create a tile showing : 1."  ;
				break;
			default :
				alert("Something unexpected happened, please reload the page.");
			}
				...
				{
					document.getElementById('bestscore').value = parseInt(localStorage.bestScore);
				} 	
		score = 0;
		...
		var startValue1 =  startingVal[Math.floor(Math.random()*4)];
		var startValue2 =  startingVal[Math.floor(Math.random()*4)];
		prepColor();
	...
	if (istest ==0){
		...
		{
			var randnumber = startingVal[Math.floor(Math.random()*4)];
	...
	function fuseUp(col)
	{
		...
		while (high < 3)
		{
			...
			else if (grid[col][high] == grid[col][high+1])
			{
	...
	function fuseLeft(line)
	{
		...
		while (left < 3)
		{
			...
			else if (grid[left][line] == grid[left+1][line])
			{
	...
	function gameOp( val, xpos, ypos, noScore)
	{
		...
		case 0:
			...
			if (((result == ruleValue*10+1))&& (noScore == 0) && (won == 0))
			{
		...
		case 1:
			var result = val * ruleValue;
			if ((result == Math.pow(ruleValue, 10))&& (noScore == 0) && (won == 0))
			{
